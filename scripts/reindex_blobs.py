...
from app.blob_service import blob_service_client
from app.ingest_service import ingest_file_into_search
from app.search_index import search_client
from scripts.create_search_index import create_index


def _is_indexed(file_name: str) -> bool:
    safe_url = file_name.replace("'", "''")
    results = search_client.search(
        search_text="*",
        filter=f"file_name eq '{safe_url}'",
        select=["id"],
        top=1,
    )
